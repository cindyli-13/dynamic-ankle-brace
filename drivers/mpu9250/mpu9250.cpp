...
void MPU9250::reset() {
  write_bit(Register::PWR_MGMT_1, static_cast<uint8_t>(PwrMgmt1Bits::H_RESET),
            1);
  write_bit(Register::USER_CTRL,
            static_cast<uint8_t>(UserCtrlBits::SIG_COND_RST), 1);
}

void MPU9250::set_sleep(bool sleep) {
  write_bit(Register::PWR_MGMT_1, static_cast<uint8_t>(PwrMgmt1Bits::SLEEP),
            sleep);
}

void MPU9250::disable_i2c(bool disable) {
  write_bit(Register::USER_CTRL, static_cast<uint8_t>(UserCtrlBits::I2C_IF_DIS),
            disable);
}

void MPU9250::set_clock_source(ClockSource clock_source) {
  write_bits(Register::PWR_MGMT_1,
...
void MPU9250::set_dlpf(AccelDLPF accel_dlpf, GyroDLPF gyro_dlpf) {
  if (accel_dlpf == AccelDLPF::NONE) {
    write_bit(Register::ACCEL_CONFIG_2,
              static_cast<uint8_t>(AccelConfig2Bits::F_CHOICE_B), 1);
  } else {
    write_bit(Register::ACCEL_CONFIG_2,
              static_cast<uint8_t>(AccelConfig2Bits::F_CHOICE_B), 0);
  ...
  if (gyro_dlpf == GyroDLPF::NONE) {
    ...
    write_bits(Register::CONFIG, static_cast<uint8_t>(ConfigBits::DLPF_CFG_LSB),
               3, static_cast<uint8_t>(gyro_dlpf));
  }
}
