...
public:
    ...
    static constexpr float ACCEL_RAW_TO_G(int16_t accel_raw, AccelFSR fsr) {
        return (static_cast<float>(2 << static_cast<uint8_t>(fsr)) / INT16_MAX) * accel_raw;
    }

    static constexpr float GYRO_RAW_TO_DPS(int16_t gyro_raw, GyroFSR fsr) {
        return (static_cast<float>(250 << static_cast<uint8_t>(fsr)) / INT16_MAX) * gyro_raw;
    }
...
private:
    ...
    enum class GyroConfigBits : uint8_t {
        FS_SEL_LSB = 3,
        F_CHOICE_B_LSB = 0
    };
    ...
    enum class AccelConfig2Bits : uint8_t {
        F_CHOICE_B = 3,
        A_DLPFCFG_LSB = 0
    };
